...
export default class Bundle {
	...
	constructor (graph: Graph, id: string, orderedModules: Module[]) {
		...
		this.entryModuleFascade = false;
		orderedModules.forEach(module => {
			if (module.isEntryPoint) {
				if (!this.entryModule) {
					this.entryModule = module;
					this.entryModuleFascade = true;
				} else {
					this.entryModuleFascade = false;
				}
			}
	...
	ensureExport (module: Module | ExternalModule, variable: Variable): string {
		...
		// we will then need an entry point fascade if this is an entry point module
		if (this.entryModuleFascade && module.bundle === this && !module.isEntryPoint) {
			this.entryModuleFascade = false;
		}
	...
	generateDependencies (entryFascade?: Module) {
		if (entryFascade) {
			this.externalModules = [];
			this.dependencies = [entryFascade.bundle];
			return;
		}
