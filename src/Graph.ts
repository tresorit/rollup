	...
	buildSingle (entryModuleId: string): Promise<Bundle> {
		...
		return this.loadModule(entryModuleId)
			.then(entryModule => {
				...
				// Phase 4 – we construct the bundle itself, generating its import and export fascades
				timeStart('phase 4');
	...
	buildChunks (entryModuleIds: string[]): Promise<{ [name: string]: Bundle }> {
		...
		return Promise.all(entryModuleIds.map(entryId => this.loadModule(entryId)))
			.then(entryModules => {
				...
				// entry point graph colouring, before generating the import and export fascades
				timeStart('phase 4');
			  ...
			  //       either as a namespace import reexported or top-level export *)
				...
				bundleList.forEach(bundle => {
					...
					if (bundle.entryModule) {
						...
						if (bundle.entryModuleFascade) {
							bundles['./' + entryName] = bundle;
							bundle.setId('./' + entryName);
						// otherwise we create a special re-exporting entry point
						// fascade bundle with no modules
						} else {
							const entryPointFascade = new Bundle(this, './' + entryName, []);
							entryPointFascade.generateEntryExports(bundle.entryModule);
							entryPointFascade.generateDependencies(bundle.entryModule);
							entryPointFascade.generateImports();
							bundles['./' + entryName] = entryPointFascade;
					...
					if (!replacement) {
						...
						module.dynamicImportResolutions[index] = replacement;
							...
							if (!this.moduleById.has(replacement)) {
								externalModule = new ExternalModule(this, replacement);
